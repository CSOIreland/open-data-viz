 ...
 * @param {*} pAppend 
 */
api.content.load = function (pSelectorContainer, pRelativeURL, pParams, pAppend, pNav_link_SelectorToHighlight, pNav_menu_SelectorToHighlight, pPushState) {
  // Default parameters
  pParams = pParams || {};
  pAppend = pAppend || false;
  pPushState = pPushState || false;
  ...
  if (pPushState) {
    history.pushState(
      {
        pRelativeURL: pRelativeURL,
        pNav_link_SelectorToHighlight: pNav_link_SelectorToHighlight,
        pNav_menu_SelectorToHighlight: pNav_menu_SelectorToHighlight,
        pParams: pParams
      },
      '',
      "?body=" + pRelativeURL
 ...
 */
api.content.navigate = function (pNavSelector, pRelativeURL, pNav_link_SelectorToHighlight, pNav_menu_SelectorToHighlight, pPageTitle, pUpdateGoogleAnalytics) {
  // Default parameters
  pNav_link_SelectorToHighlight = pNav_link_SelectorToHighlight || null;
  pNav_menu_SelectorToHighlight = pNav_menu_SelectorToHighlight || null;
  pPageTitle = pPageTitle || document.title;
  pUpdateGoogleAnalytics = pUpdateGoogleAnalytics || false;
  // Validate the Relative URL
  var uri = new URI(pRelativeURL);
  if (uri.is("relative") === false)
    return;
 ...
 */
    $.ajax({
      ...
      async: false,
      success: function (response) {
        //create a history entry with a state object containing entity details for use later
        history.pushState(
          {
            pRelativeURL: pRelativeURL,
            pNav_link_SelectorToHighlight: pNav_link_SelectorToHighlight,
        ...
        //TODO This function is local to application, will not work if we migrate the api library back to jsdeliver
        app.plugin.ga.set();
      }
    });
 ...
 * @param {*} pParams 
 */
api.content.goTo = function (pRelativeURL, pNav_link_SelectorToHighlight, pNav_menu_SelectorToHighlight, pParams, pPushState) {
  // Default parameters
  pNav_link_SelectorToHighlight = pNav_link_SelectorToHighlight || null;
  pNav_menu_SelectorToHighlight = pNav_menu_SelectorToHighlight || null;
  pParams = pParams || {};
...
/**
 * Execute a static Ajax Request
 * @param {*} pUrl
 * @param {*} pCallback
 * @param {*} pAjaxParams
 */
api.ajax.static = function (pUrl, pCallback, pAjaxParams) {
  // Default AJAX parameters
  pAjaxParams = pAjaxParams || {};
  pAjaxParams.method = pAjaxParams.method || 'GET';
  pAjaxParams.dataType = pAjaxParams.dataType || 'json';
 ...
 */
api.uri.getSearch = function (pURL) {
  // Default parameters
  pURL = pURL || window.location.href;
